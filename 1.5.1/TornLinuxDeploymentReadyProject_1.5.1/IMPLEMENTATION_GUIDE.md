...
## 1.3.15 lock-in fixes
This release bakes in the manual fixes confirmed during 1.3.14 testing:

- `prepare-live-build.sh` now stages the packaged app using:
  - `cp -a "$APP_DIR"/. "$TARGET_DIR"/`
- the prepare step invokes asset stamping through:
  - `bash ./scripts/stamp-assets.sh`
- preflight stale-version detection now checks only staged branded assets:
  - `live-build/config/includes.chroot/usr/share/tornlinux`
  - `live-build/config/includes.chroot/usr/share/plymouth/themes/tornlinux`
- preflight no longer scans the full packaged app tree for numeric false positives from SVG paths, licenses, or historical docs

## Canonical build procedure (1.3.15)
```bash
cd ~
unzip TornLinuxDeploymentReadyProject_1.3.15.zip
cd TornLinuxDeploymentReadyProject_1.3.15
...
## 1.3.16 VM-safe serial debug logging
This release adds debug visibility for VM testing without exposing a visible terminal to users.

### What changed
- `tornlinux-session-start` now logs to:
  - `$HOME/.tornlinux-session.log`
  - and `/dev/ttyS0` when available
- `tornlinux-electron-launch` now logs to:
  - `$HOME/.tornlinux-electron.log`
  - and `/dev/ttyS0` when available

### Why
This allows QEMU/VM tests to surface session and Electron launcher logs directly in the host terminal when started with serial output enabled, while keeping the normal user-facing desktop free of debug terminals.

### QEMU debug recommendation
Use:
```bash
qemu-system-x86_64 \
  -m 4096 \
  -smp 2 \
  -cdrom live-build/TornLinux-1.3.16.iso \
  -boot d \
  -enable-kvm \
  -display gtk,gl=off \
  -serial mon:stdio
...
## Canonical build procedure (1.3.16)
```bash
cd ~
rm -rf TornLinuxDeploymentReadyProject_1.3.16
unzip TornLinuxDeploymentReadyProject_1.3.16.zip
cd TornLinuxDeploymentReadyProject_1.3.16
...
## 1.3.17 network onboarding UI scope
This release compiles the previously approved UI work into the project:

- moved Layout out of the header and into Settings
- added a live Online / Offline network control to the header in Layout's previous position
- Online state uses a controlled glow treatment
- added a first-boot offline gate with:
  - Open Network Settings
  - Retry
  - Continue Offline
- added a Network section in Settings as a secondary recovery path
- added a renderer network status hook and Electron bridge for connectivity checks


## 1.4.0 milestone scope
This release consolidates the validated system fixes from the 1.3.x line and introduces the first integrated user-experience layer.

Included in 1.4.0:
- deterministic packaged-app staging with runtime execute permissions
- restored master asset pipeline using corrected splash and wallpaper sources
- Plymouth splash refined to use an animated back-and-forth scroller instead of static initialization text
- API key integration with immediate refresh on settings save
- temporary seeded API key for this build only, with auto-clear behavior in later versions
- Settings cleanup removing user-editable URLs, Discord size, and refresh-rate controls
- header network status control (Offline / Online)
- Settings-based audio slider and Bluetooth entry point
- compact TCT readout in the header
- first-boot offline gate and persistent network recovery path
- VM-safe serial debug logging without a visible terminal

### Human rebuild steps for 1.4.0
```bash
cd ~
rm -rf TornLinuxDeploymentReadyProject_1.4.0
unzip TornLinuxDeploymentReadyProject_1.4.0.zip
cd TornLinuxDeploymentReadyProject_1.4.0
...
## 1.4.1 tool availability lock-in
This release explicitly adds the system tools expected by the TornLinux UI and its fallbacks.

### Added package guarantees
- `network-manager`
...
### UI control to system command mapping
- Header Network / offline gate:
  - primary: `nm-connection-editor`
  - fallback: `xterm -e nmtui`
- Settings Bluetooth icon:
  - primary: `blueman-manager`
  - fallback path can use BlueZ stack (`bluetoothctl`) if later required
- Settings volume slider:
  - runtime backend should use PipeWire/Pulse-compatible tools such as `pactl`
  - fallback GUI path: `pavucontrol`

## Canonical build procedure (1.4.1)
```bash
cd ~
rm -rf TornLinuxDeploymentReadyProject_1.4.1
unzip TornLinuxDeploymentReadyProject_1.4.1.zip
cd TornLinuxDeploymentReadyProject_1.4.1
...
## 1.5.0 entry flow and persistence
This release introduces the next-stage boot UX:

### Entry flow
- splash transitions into a dedicated entry decision screen
- entry screen offers:
  - Run Live
  - Install TornLinux
- entry screen includes an auto-continue countdown
- last selected mode is persisted and used as the next default mode

### Live path
- Run Live enters a first-use landing screen
- the landing screen handles:
  - network state
  - API key setup
  - continue into system
- first-use completion is persisted

### Installer path
- Install opens an installer screen
- installer screen can launch the installer backend
- current backend target is `calamares`

### Runtime API popup
- the old first-run popup has been repurposed
- it now appears as an API key gate when the system reaches runtime without a valid API key


## 1.5.1 immediate correction lock-in
This patch locks in two required fixes that had been identified during rebuild/testing and should not remain manual:

- `electron/preload.cjs` now exposes:
  - `launchNetworkSettings`
  - `getNetworkStatus`
- `scripts/prepare-live-build.sh` now applies:
  - `chmod -R 755 "$TARGET_DIR"`
  after staging the packaged app into the live-build payload

These are no longer operator-side reminders. They are part of the source package.
