...
import type { AppConfig, AppSettings, BootIntent, ConfigStatus } from '@shared/types';
...
export function App() {
  ...
  const saveApiKeys = async ({ tornApiKey, tornStatsApiKey }: { tornApiKey: string; tornStatsApiKey: string }) => {
    const tornKey = String(tornApiKey || '').trim();
    const tornStatsKey = String(tornStatsApiKey || '').trim();
  ...
  const openNetworkSettings = async () => {
    ...
    setActionMessage(result?.ok ? 'Opening network settings' : 'Network settings unavailable in this session');
  };

  const openBluetoothSettings = async () => {
    const result = await window.tornlinux?.launchBluetoothSettings?.();
    setActionMessage(result?.ok ? 'Opening bluetooth settings' : 'Bluetooth settings unavailable in this session');
  };
  ...
  if (bootStage === 'landing') {
    return (
      <FirstUseLanding
        isOnline={isOnline}
        hasTornApiKey={Boolean(configStatus?.hasTornApiKey)}
        hasTornStatsApiKey={Boolean(configStatus?.hasTornStatsApiKey)}
        initialTornApiKey={config.tornApiKey}
        initialTornStatsApiKey={config.tornStatsApiKey}
    ...
    <div className="tla-root">
      ...
      <main className="tla-content" style={{ gridTemplateColumns: layout.resolvedMode === 'split' ? `minmax(0, 1fr) minmax(340px, ${layout.discordWidth}px)` : 'minmax(0, 1fr)' }}>
        <section className="tla-pane tla-pane--torn">
          {!configStatus?.hasTornApiKey && <div className="tla-banner">No valid Torn API key configured. Add one in Settings or continue in limited mode.</div>}
